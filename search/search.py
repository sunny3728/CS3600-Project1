...
def findPathFromParents(start, goal, parents):
...
def depthFirstSearch(problem):
    ...
    while not problem.isGoalState(current[0]) and open:
        ...
        for successor in successors:
            if successor not in parents.keys() or parents[successor] != start:
                parents[successor] = current
        while current[0] in closed:
            current = open.pop()

    return findPathFromParents(start, current, parents)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    current = start
    open = util.PriorityQueue()
    open.push(current, heuristic(problem.getStartState(), problem))
    closed = []
    parents = {}
    cost = {start: heuristic(problem.getStartState(), problem)}

    while not problem.isGoalState(current[0]):
        closed.append(current[0])
        successors = problem.getSuccessors(current[0])
        for successor in successors:
            open.push(successor, cost[current] + successor[2] + heuristic(successor[0], problem))
            cost[successor] = cost[current] + successor[2]
            if successor not in parents.keys():
                parents[successor] = current
        while current[0] in closed:
